...
class TestTorchGMMInitialization:
    ...
    def test_init_supported_covariance_types(self, covariance_type):
        """Test that every supported covariance type fits and exposes full covariances."""
        X = np.random.randn(50, 2)
        gmm = TorchGMM(n_components=2, covariance_type=covariance_type, device="cpu").fit(X)
        assert gmm.covariances_.shape == (2, 2, 2)

    def test_init_unsupported_covariance_type(self):
        """Test that unsupported covariance types raise ValueError when fitting."""
        with pytest.raises(ValueError, match="Unknown covariance_type"):
            TorchGMM(n_components=2, covariance_type="bad", device="cpu").fit(
